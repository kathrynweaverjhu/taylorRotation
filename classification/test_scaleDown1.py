...
rsearch_best_estimator = joblib.load("/home-3/kweave23@jhu.edu/work/users/kweave23/out/{}".format(model_ext))
params = rsearch_best_estimator.get_params()
...
def score(X, y, padding, alpha, filters, kernel_size, activation, pool_size, strides, epochs, needsCompiled, inputNodes, outputNodes, hiddenNodes, batchSize, steps_per_epoch, estimator):
    batchSize = batchSize
    filters = filters
...
'''Get metrics'''    
accuracy, confusion_matrix, classification_report, precision, recall, fpr, tpr, roc_auc, average_precision = score(merged_test_arrays, labels, **params, rsearch_best_estimator)
...
colors = itertools.cycle(['slategray', #1
                          'darkorchid', #2
                          'aqua',       #3
                          'darkorange', #4
                          'cornflowerblue', #5
