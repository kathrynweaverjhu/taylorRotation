...
'''Usage: ./ClassificationModel_scaleDown.py'''


'''loading saved numpy arrays of annotated data'''
with np.load("savedMatrices.npz") as npzfile:
...
print("loaded saved numpy arrays")
print("number of samples: " , labels.shape[0])

scaling_factor = 1e3
indices = np.random.choice(int(cellTypeIndex.shape[0]), size=int(scaling_factor))
cellTypeIndex = cellTypeIndex[indices]
labels = labels[indices]
sequences = sequences[indices]
RNA_seq = RNA_seq[indices]
ATAC_seq = ATAC_seq[indices]
print("scaled down with random sampling")
print(int(labels.shape[0]) == int(scaling_factor))
...
f = open('testArrays_scaleDown1.npz', 'wb')
...
#np.savez(f, Y_cellTypeIndex_train_full = Y_cellTypeIndex_train_full, 
#X_labels_train_full = X_labels_train_full, merged_train_arrays_full = 
#merged_train_arrays)
#f.close()
...
filename = 'VISION_classification_RSCV_scaleDown1_best'
