...
parser=ap.ArgumentParser(description='autoencoder with 3dim code layer')
...
'''fit'''
history = m.fit_generator(generator(merged_train_arrays, epochs, batchSize), epochs=epochs, steps_per_epoch=steps_per_epoch, workers=0, callbacks=[cp_callback])
...
figname='autoencoder_{}.mp4'.format(plotName)
anim.save(figname, fps=30, extra_args=['-vcodec', 'libx264'])
